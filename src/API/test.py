...
# dates = ["2020-01-19", "2020-02-19", "2020-03-19", "2020-04-19", "2020-05-19", "2020-06-19", "2020-07-19", "2020-08-19",
#          "2020-09-19", "2020-10-19", "2020-11-19", "2020-12-19", "2021-01-19"]
# months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']

dates = ["2020-07-19", "2020-08-19"]
months = ['Jul']
...
for i in range(len(dates[:-1])):
    ...
    date_till = dates[i + 1]
    month = months[i]
    time_handler = TimeHandler(date_from, date_till, datetime_unit_of_measure, period_value)
    ...
    for num in range(1, 55):
        ...
        for using_Kalman_Filter in using_Kalman_Filter_values:
            ...
            path = "Test_results/" + "{}".format(month)
            isFile = os.path.isdir(path)
            if isFile is False:
                os.mkdir(path)
            ...
            actual_data_df = time_handler. \
                get_real_data_for_comparing_with_forecast(list_of_datetime, list_of_measurements,
                                                          date_from_for_actual_data=time_handler.date_till, steps=steps)
            ...
            if data_df.empty:
                ...
                if using_Kalman_Filter:
                    ...
                    none_values_after.append(0)
                continue

            if using_Kalman_Filter:
                total_values.append(len(data_df))
                none_values.append(int(data_df.isna().sum()["Measurements"]))
            ...
            if data_df.isnull().values.any() or actual_data_df.isnull().values.any():
                ...
                if data_df.isnull().values.any() or actual_data_df.isnull().values.any():
                    data_df, actual_data_df = DataFiller(data_df, actual_data_df, station_clusters_df, station_code,
                                                         steps, time_handler, sql_client, station_locations_rec,
                                                         use_second_data_type=use_second_data_type).fill_data_none_values(value)
                if using_Kalman_Filter:
                    none_values_after.append(int(data_df.isna().sum()["Measurements"]))
            ...
            plt.title("{} - {}, {} {} points".format(date_from, date_till, station_code, len(data_df)))
            ...
            #     forecast_ar_df = DataHandler.duplicate_dataframe_using_real_datafrme(actual_data_df, forecast_ar)
            ...
            #     forecast_ma_df = DataHandler.duplicate_dataframe_using_real_datafrme(actual_data_df, forecast_ma)
            ...
            #
            if using_Kalman_Filter:
                models_with_KF[station_code] = [arima_model_order]
                rmse_results_with_KF[station_code] = [arima_rmse]
            else:
                models_without_KF[station_code] = [arima_model_order]
                rmse_results_without_KF[station_code] = [arima_rmse]

            print(plot_text_result + "\n")

            plt.legend(plot_legend_result)

            plt.title("{} - {}, forecast for {} points".format(date_from, date_till, steps))
            plt.xlabel("Date and time")
            plt.ylabel("Dew point")
    ...
    results_with_KF_df = pd.DataFrame(rmse_results_with_KF, index=["ARIMA"])
    results_without_KF_df = pd.DataFrame(rmse_results_without_KF, index=["ARIMA"])
    models_with_KF_df = pd.DataFrame(models_with_KF, index=["ARIMA"])
    models_without_KF_df = pd.DataFrame(models_without_KF, index=["ARIMA"])
    station_values_df = pd.DataFrame.from_dict({"Total values": total_values, "None values": none_values,
                                                "None values after filling": none_values_after}, orient="index",
                                               columns=stations)

    writer = pd.ExcelWriter("Test_results/results_{}.xlsx".format(month), engine='xlsxwriter')
    ...
    station_values_df.to_excel(writer, sheet_name='station_values')
    writer.save()
