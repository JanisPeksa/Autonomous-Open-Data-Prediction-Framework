...
class MA_main:
    ...
    def get_ma_order(data_series):
        ...
        for q in q_values:
            ...
            try:
                ma_model = ARIMA(data_series, order).fit(disp=0)
                aic = ma_model.aic
    ...
    @staticmethod
    def make_ma_forecast(ma_model, data_series, steps):
        ma_model = ARIMA(data_series, order=ma_model).fit(disp=False)

        forecast_list = ma_model.forecast(steps=steps)[0].tolist()
    ...
    def get_all_possible_ma_models_with_data_points(self):
        if self.optimize:
        ...
        ma_models_order_and_data_points_dict = dict()
        ...
        for data_points in data_point_range:
            ...
            order = self.get_ma_order(data_series_copy)
            if order is None:
                continue
            else:
                ma_models_order_and_data_points_dict[data_points] = order

        return ma_models_order_and_data_points_dict

    def get_ma_forecast(self):

        ma_models_order_and_data_points_dict = self.get_all_possible_ma_models_with_data_points()
        ...
        for data_points, order in ma_models_order_and_data_points_dict.items():
            ...
            forecast_list = self.make_ma_forecast(order, data_series_copy, self.steps)
        ...
        rmse_df['order'] = ma_models_order_and_data_points_dict.values()
        ...
        ma_model_order = self.get_ma_order(data_series_copy)

        if ma_model_order is None:
            return [None, None, None, None]

        forecast = self.make_ma_forecast(ma_model_order, data_series_copy, steps)

        return [data_points_for_forecast, ma_model_order, steps, forecast]